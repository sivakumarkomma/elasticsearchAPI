...
public class GeoBounds {
  public static void main(String[] args) throws IOException {
    ...
    searchSourceBuilder.aggregation(AggregationBuilders.geoBounds("gb_sa_fileSize").field("sa_indexID").wrapLongitude(true));
    searchRequest.source(searchSourceBuilder);
    SearchResponse searchResponse = highLevelClient.search(searchRequest);

    System.out.println(" GeoBounds  " + searchResponse.getAggregations().get("gb_sa_fileSize").toString());
    System.out.println(" GeoBounds  " + searchResponse.getAggregations().get("gb_sa_fileSize").getName());
    System.out.println(" GeoBounds  " + searchResponse.getAggregations().get("gb_sa_fileSize").getType());
    ParsedExtendedStats parsedExtendedStats = searchResponse.getAggregations().get("gb_sa_fileSize");
